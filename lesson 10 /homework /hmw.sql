...
SELECT o.OrderID, o.OrderDate,p.PaymentDate, Amount
FROM Payments p
JOIN Orders o ON p.PaymentID = p.PaymentID
...
SELECT s.StudentName, c.CoursesName, e.Enrollment
FROM Students s 
INNER JOIN StudentName ON s.StudentName = CourseName
...
🔁 Expected Output: FristName, LastName, Quantity
...
SELECT e.EmployeeName, d.DepartmentName
FROM Employees e 
JOIN Departments ON e.DepartmentID = d.DepartmentID
...
---11Using the Products and Sales tables,WHERE s.ProductID IS NULL;
...
SELECT e.EmployeeID, d.DepartmentName, e.Salary
FROM Employees e
JOIN Departments d ON e.DepartmentID = d.DepartmentID
...
SELECT o.OrderID, o.OrderDate, p.PaymentAmount
FROM Orders o
...
SELECT e.EmployeeID, Avg(Salary) AS AvgSalary
FROM Employees e 
...
FROM Products p
LEFT JOIN Categories c ON p.CategorieID = c.CategoryID
...
WHERE e1.EmployeeID < e2.EmployeeID 
  ...
  ---23Using the Employees and Departments tables, write a query to return employees who work in departments whose name starts with the letter 'M'.
🔁 Expected Output: EmployeeName, DepartmentName
...
WHERE c.CategoryName IN ('Electronics', 'Furni
