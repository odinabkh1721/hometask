...
SELECT 
    COALESCE(phone, email, address) AS First_NonNull_Contact
FROM 
    PERSON 
...
SELECT 
    column_name,
    GROUP_CONCAT(another_column ORDER BY another_column SEPARATOR ', ') AS concatenated_values
FROM 
    DMLTable
GROUP BY 
    column_name;
	...
	13)Find all employees whose names (concatenated first and last) contain the letter "a" at least 3 times.
...
FROM 
    employees
WHERE 
    LENGTH(REPLACE(CONCAT(FIRST_NAME,LAST_NAME), 'a', '')) <= LENGTH(CONCAT(FIRST_NAME, LAST_NAME) - 3;


	14)he total number of employees in each department and the percentage of those employees who have been with the company for more than 3 years(Employees)
...
SELECT
    original_string,
    -- Katta harflar
    (SELECT STRING_AGG(SUBSTRING(original_string, number, 1), '') 
     FROM master.dbo.spt_values 
     WHERE number BETWEEN 1 AND LEN(original_string)
     AND SUBSTRING(original_string, number, 1) LIKE '[A-Z]') AS uppercase_letters,
    -- Kichik harflar
    (SELECT STRING_AGG(SUBSTRING(original_string, number, 1), '') 
     FROM master.dbo.spt_values 
     WHERE number BETWEEN 1 AND LEN(original_string)
     AND SUBSTRING(original_string, number, 1) LIKE '[a-z]') AS lowercase_letters,
    -- Raqamlar
    (SELECT STRING_AGG(SUBSTRING(original_string, number, 1), '') 
     FROM master.dbo.spt_values 
     WHERE number BETWEEN 1 AND LEN(original_string)
     AND SUBSTRING(original_string, number, 1) LIKE '[0-9]') AS numbers,
    -- Boshqa belgilar
    (SELECT STRING_AGG(SUBSTRING(original_string, number, 1), '') 
     FROM master.dbo.spt_values_
    ...
    student_name,
    score,
    SUM(score) OVER (ORDER BY student_id) AS cumulative_sum
FROM 
    Students;
